...
void USART2_IRQHandler(void) {
    if (USART_GetITStatus(USART2, USART_IT_RXNE) != RESET) {
        ...
        if (USART2_RxLen < USART2_BUF_SIZE - 1) {
            ...
            USART2_RxLen = USART2_BUF_SIZE - 1; // 防止溢出
        }

        /* 检测结束符 \r\n */
        if (USART2_RxLen >= 2) {
            if (USART2_RxBuffer[USART2_RxLen - 2] == '\r' && USART2_RxBuffer[USART2_RxLen - 1] == '\n') {
                USART2_RxFlag = 1;
            }
        }
        // 清除中断标志
...
static uint8_t check_wifi_format(uint8_t* buf, uint16_t len) {
    ...
    // 1. 检查开头是否为!
    USART_SendString(USART1, "start send buf[0]\r\n");
    USART_SendString(USART1, (char*)buf);
    //USART_SendData(USART1, buf[0]);
    USART_SendString(USART1, "\r\n");
    USART_SendString(USART1, "end send buf[0]\r\n");
    if (buf[0] != '!') {
